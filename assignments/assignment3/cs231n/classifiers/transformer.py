...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # We have features of size (N,D) and we want to project them in the word space, we use visual_projection for that
        projected_features = self.visual_projection(features)
        # Now it's size (N, W)
        projected_features = projected_features.unsqueeze(1)

        # Now embed the caption

        embedded_captions = self.embedding(captions)
        embedded_captions = self.positional_encoding(embedded_captions)

        # Prepare target mask to mask out future timesteps
        tgt_mask = torch.tril(torch.ones((T, T), device=features.device, dtype=torch.bool))

        # Apply the transformer decoder on text and image embeddings with the target mask
        transformer_output = self.transformer(tgt=embedded_captions, memory=projected_features, tgt_mask=tgt_mask)

        # Project the transformer output to scores per token
