...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        dh, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(dscores, cache_vocab)
    ...
    def sample(self, features, max_length=30):
        ...
        word_idx = np.full((N,), self._start, dtype=np.int32)

        for t in range(max_length):
            word_vec, _ = word_embedding_forward(word_idx, W_embed)
            
            if self.cell_type == "rnn":
                h, _ = rnn_step_forward(word_vec, h, Wx, Wh, b)
            elif self.cell_type == "lstm":
                h, c, _ = lstm_step_forward(word_vec, h, c, Wx, Wh, b)
            
            scores, _ = temporal_affine_forward(h, W_vocab, b_vocab)
            word_idx = np.argmax(scores, axis=2).flatten()
            captions[:, t] = word_idx
