...
def discriminator_loss(logits_real, logits_fake):
    ...
    real_loss = criterion(logits_real, true_labels)
    fake_loss = criterion(logits_fake, fake_labels)
...
def generator_loss(logits_fake):
    ...
    # The generator wants the discriminator to output 1 for fake samples
    true_labels = torch.ones_like(logits_fake)

    criterion = nn.BCEWithLogitsLoss()

    # Compute the loss using binary_cross_entropy_with_logits
    loss = criterion(logits_fake, true_labels)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    # Compute the real and fake losses using the least-squares loss
    real_loss = 0.5 * torch.mean((scores_real - 1) ** 2)
    fake_loss = 0.5 * torch.mean(scores_fake ** 2)

    # Combine the real and fake losses to get the total loss
    loss = real_loss + fake_loss
